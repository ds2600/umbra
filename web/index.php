...
body::after{content:'';position:fixed;inset:0;background-image:radial-gradient(1px 1px at 15% 20%,rgba(255,255,255,0.35) 0%,transparent 100%),radial-gradient(1px 1px at 72% 8%,rgba(255,255,255,0.25) 0%,transparent 100%),radial-gradient(1px 1px at 40% 45%,rgba(255,255,255,0.2) 0%,transparent 100%),radial-gradient(1px 1px at 88% 55%,rgba(255,255,255,0.3) 0%,transparent 100%),radial-gradient(1px 1px at 5% 70%,rgba(255,255,255,0.2) 0%,transparent 100%),radial-gradient(1.5px 1.5px at 50% 15%,rgba(192,142,45,0.5) 0%,transparent 100%),radial-gradient(1.5px 1.5px at 25% 60%,rgba(133,89,142,0.4) 0%,transparent 100%);pointer-events:none;z-index:0;}
...
.field input{width:100%;background:rgba(0,0,0,0.35);border:1px solid var(--border);border-radius:var(--radius-sm);padding:.75rem 1rem;color:var(--text);font-family:'Inter',sans-serif;font-size:.9rem;font-weight:300;outline:none;transition:border-color .2s;}
...
.ingest-key-meta h3{font-size:.8rem;font-weight:500;letter-spacing:.06em;color:var(--text);margin-bottom:.25rem;}
...
.key-input-row input{flex:1;min-width:160px;background:rgba(0,0,0,0.4);border:1px solid var(--border);border-radius:var(--radius-sm);padding:.6rem .85rem;color:var(--text);font-family:'Inter',sans-serif;font-size:.82rem;font-weight:400;letter-spacing:.04em;outline:none;transition:border-color .2s;}
...
.plat-key-input{width:100%;background:rgba(0,0,0,0.3);border:1px solid var(--border);border-radius:var(--radius-sm);padding:.5rem .7rem;color:var(--text);font-family:'Inter',sans-serif;font-size:.77rem;font-weight:300;outline:none;transition:border-color .2s;}
  ...
  <div class="login-card">
    <div class="login-logo">
      <svg class="eclipse-logo" viewBox="0 0 72 72" fill="none">
        ...
        <path d="M36 64l2 6-2-3-2 6Z" fill="#C08E2D" opacity=".6"/>
...
<div class="dash">
  ...
  <div class="dash-header">
    <div class="dash-brand">
      <svg viewBox="0 0 72 72" fill="none">
        ...
        <path d="M36 64l1.5 5L36 66l-1.5 5Z" fill="#C08E2D" opacity=".55"/>
  ...
  <div class="section">
    ...
    <svg id="flow-svg" viewBox="0 0 700 170" style="width:100%;height:auto;display:block;" xmlns="http://www.w3.org/2000/svg">
      <defs>
...
<div class="modal-backdrop" id="modal">
  <div class="modal">
    <h3>Apply Changes</h3>
    <p>This will reload nginx and the stream relay. Active platform pushes will briefly reconnect. Your camera ingest connection will not be interrupted.</p>
...
document.getElementById('btn-regen-key').addEventListener('click', () => {
  ...
  document.getElementById('ingest-key-input').value = key;
  document.getElementById('hint-key').textContent   = key.slice(0,8)+'…';
});

function updatePlatformCards() {
  ['yt','fb','tt'].forEach(p => {
    document.getElementById('card-'+p).classList.toggle('disabled', !document.getElementById('toggle-'+p).checked);
  });
  updateDiagram();
}
...
function updateDiagram() {
  const active = currentIngestState === 'green' || currentIngestState === 'yellow';
  setLine('line-ingest', active);
  const colors = {yt:'rgba(255,80,80,0.9)',fb:'rgba(100,150,240,0.9)',tt:'rgba(200,200,200,0.85)'};
  ['yt','fb','tt'].forEach(p => {
    const on = active && restreamEnabled && document.getElementById('toggle-'+p).checked;
    setLine('line-'+p, on);
    const dot = document.getElementById('dot-'+p);
    dot.setAttribute('fill', on ? colors[p] : 'rgba(255,255,255,0.08)');
    dot.setAttribute('opacity', on ? '1' : '0.3');
  });
}
  ...
  try {
    ...
    if (d.connected) {
      ...
      if (d.bitrate_kbps < 500) {
        currentIngestState = 'yellow'; dot.classList.add('yellow');
        label.textContent = 'Unstable'; diagDot.setAttribute('stroke','rgba(212,160,23,0.3)');
      } else {
        currentIngestState = 'green'; dot.classList.add('green');
        label.textContent = 'Connected'; diagDot.setAttribute('stroke','rgba(39,174,96,0.3)');
      }
      brate.textContent = mbps+' Mbps';
    } else {
      currentIngestState = 'red'; dot.classList.add('red');
      label.textContent = 'No signal'; brate.textContent = '';
...
async function pollStats() {
  try {
    ...
    setStatVal('stat-mem',    Math.round(d.mem_mb),           'MB');
    setBar('bar-mem',         d.mem_pct);
    setStatVal('stat-netin',  (d.net_in_mbps||0).toFixed(1),  'Mbps');
    setBar('bar-netin',       Math.min((d.net_in_mbps||0)*5,100));
    setStatVal('stat-netout', (d.net_out_mbps||0).toFixed(1), 'Mbps');
...
document.getElementById('modal-confirm').addEventListener('click', async () => {
  ...
  try {
    ...
    if (d.success) {
      ...
      const k = document.getElementById('ingest-key-input').value;
  ...
  } catch(e) { status.textContent = 'Network error'; status.className = 'save-status err'; }
